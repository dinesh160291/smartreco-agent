...
| POL-CONF-001 | Confidence contribution | v1.15: Weak +0.05, Medium +0.10, Strong +0.20, Very Strong +0.30 per supporting **action**, damped within its identity by POL-CONF-002 and combined across identities by noisy-OR. The flat diversity increment (+0.10 per distinct pattern beyond the first) is retired: a second pattern contributes its own independent terms, so the combination already pays for diversity, and paying twice let a concept that shares another's Evidence outrank the concept whose pattern produced it (Decision #091) |
| POL-CONF-002 | Diminishing returns | v1.15: the damped unit is the supporting **action**, not the Evidence record (Decision #091). An identity is pattern + strength + kind of event; the nth action counted into an identity contributes 50% of the one before it. An action already counted contributes **nothing**, so a run that re-reported its session without observing anything new cannot move confidence. A different strength opens a new identity at full class value, which is how escalation — including the multi-session clause, where no new action need arrive — still registers. v1.4 counted the Evidence record under the same identity (Decision #054); because session-window patterns re-report their whole session, that damped records carrying genuinely new actions and made the running sum a convergent series whose limit was set by browsing texture — a subject supported by 46 events converged on 0.4999… against POL-REQ-001's 0.5 and could never publish. v1 read the identity's event types as a multiset (Decision #036), under which session-window patterns never repeated at all and confidence tracked run count |
...
| POL-REC-002 | Ranking | Rank by weighted coverage: Critical ×3, High ×2, Medium ×1, Low ×0.5. Within a Requirement each required Capability counts for its association weight — Primary 1.0, Secondary 0.6, Supporting 0.3 — not one apiece. That figure is the published coverage; ranking is on a separate match score — coverage × 0.6 for a candidate outside every category the shopper has been researching, with no subject held the term drops out. The discount never reaches the published coverage, which keeps its arithmetic definition, so a ranked list is not always ordered by the figure beside it (Decision #078). The categories a candidate is measured against are those of every subject held above 0.2 — deliberately below POL-REQ-004's 0.5 anchoring bar, because "what kinds of product am I looking at" is a weaker claim than "what is this journey about" (Decision #082). Tie-break on total capability count, then Product ID |
...
| POL-TRIG-003 | AI budgets | v1.3: Tier 1: **30** calls/user/day; Tier 2: **40** calls/user/day. v1 was 10 / 20 — raised alongside the faster cadence so a demo does not exhaust the budget mid-journey (Decision #048). **v1.17:** instance-wide daily ceilings beside the per-user ones — Tier 1 **2000**, Tier 2 **3000**. Per-user budgets multiplied by a thousand registered users are a bill nothing caps; reaching either ceiling degrades the slow path exactly as the per-user budget does, so deterministic reasoning continues and only the words stop (Decision #100) |
| POL-TRIG-004 | Material change | New/removed Requirement, priority band change, stage change, or top-candidate change |
| POL-TRIG-005 | Run concurrency | At most one in-flight workflow run per user; a trigger arriving during a run is recorded as SKIP (already-running) and its events remain accumulated for the next evaluation. **v1.16:** `max_concurrent_runs` (3) bounds the *instance* as well as the user — background reasoning is shed, not queued, once that many runs are in flight |
| POL-RATE-001 | Arrival rate | **v1.17:** per-caller ceilings on event ingestion (120/min) and auth (20/min); 429 with Retry-After. Caller identified from the proxy's client-IP header; the limiter itself is bounded at 4096 tracked callers |
| POL-BACKUP-001 | System-of-record backup | **v1.17:** the relational store copied hourly via SQLite's backup API — never a file copy, which omits the write-ahead log — keeping the newest 12 |
...
| POL-JRES-001 | Journey resolution | Reuse ACTIVE journey when resolution score ≥ 0.6 (weights: topic 0.4, behavioral 0.3, time-decay 0.3); reactivate DORMANT ≥ 0.7; else create new. Signal functions defined in Chapter 12 — Signal Computation (topic = Jaccard over entity sets; behavioral = cosine over event-type histograms; time-decay half-life 7 days). **min_session_events = 5** — ownership is deferred until a session has settled (Chapter 12 § Session Settlement, Decision #041); cold start exempt  **fork_min_events = 5** (Decision #056): ownership is reconsidered for each settled block of new events in an already-owned session, so a shopper who changes subject mid-session gets a second journey; a smaller block inherits. **recent_window_events = 15**: the journey forks when its established subject (dominant before the window) is absent from the window. Tested on the Domain Pack's subject-bearing concepts, not entity overlap (Decisions #056, #057) |
