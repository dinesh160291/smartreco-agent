...
**Behaviour.** Deterministic and model-free: the same query always returns the same products in the same order, and the ordering is explainable without inspecting a model. This is *not* the Semantic Retrieval Engine — Chapter 20's vector index answers "what does this person's behaviour imply?", while the search box answers "what did this person ask for". **One exception, and only one** (Decision #089, spec: `docs/implementation/semantic-search-spec.md`): a query that returns **zero** lexical results may be answered from the index instead of with an empty page. Everything specified in this section is unchanged for every query that returns something, which is what makes the exception safe — the fallback is unreachable for any query the deterministic path can serve.
...
**Semantic fallback (zero results only).** When the lexical path finds nothing, the shopper's query is embedded and matched against the vector index. POL-SRCH-001's measured `min_similarity` gates the **top hit** — if the best match is not close enough the query has no answer and the empty state below renders unchanged — and the page is then filled from hits within `neighbour_band` of that top hit. Applying the gate per-hit instead left pages holding a single product, which is a dead end for both the shopper and the reasoning engines (Decision #090). The result line then reads **"No exact matches for "…" — showing N products with related capabilities."** The wording never claims a match: "related capabilities" is a claim the catalog supports, "best match" is not. Ordering is by similarity then Product ID, so the list is replayable. **Similarity scores never appear on this surface** — they are an internal quantity, negative on the Chapter 20 scale, and printing one invites reading it as a percentage fit; they belong to the admin-gated Reasoning Panel. Cost is bounded by POL-SRCH-002, and every failure — gateway down, index empty, budget spent — renders the ordinary empty state rather than an error.
