...
This is a per-product test, not a per-pane one. The Docs pane carries whatever topic the product's own capabilities imply, so it qualifies on an AI product and is blank on an identity product whose topic is `sso`. Pricing and Integrations are always blank: their patterns reach Strong on breadth or recurrence, not on time. Turning a topic on is a Domain Pack change — add the dwell clause and the constant, and the surfaces follow with no edit here.
